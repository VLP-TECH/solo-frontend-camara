/**
 * Sube el CSV componentes_indicadores.csv a Supabase (tabla public.componentes_indicadores).
 *
 * Uso:
 *   node scripts/upload-componentes-indicadores.mjs [ruta/al/archivo.csv]
 *
 * Si no pasas ruta, usa: ~/Downloads/componentes_indicadores.csv
 */
...
function parseRow(line) {
  ...
  return {
    id,
    id_indicador: Number.isNaN(id_indicador) ? null : id_indicador,
    descripcion_dato: descripcion_dato === 'nan' ? null : descripcion_dato,
    fuente,
    rol,
  };
}
...
async function main() {
  const csvPath = process.argv[2] ?? resolve(process.env.HOME || process.env.USERPROFILE, 'Downloads', 'componentes_indicadores.csv');

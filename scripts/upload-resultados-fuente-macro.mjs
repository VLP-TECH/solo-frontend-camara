/**
 * Sube el CSV resultados_fuente_macro.csv a Supabase (tabla public.resultados_fuente_macro).
 *
 * Uso:
 *   node scripts/upload-resultados-fuente-macro.mjs [ruta/al/archivo.csv]
 */
...
function parseRow(line) {
  ...
  const id_dato_macro = parseInt(parts[1], 10);
  if (Number.isNaN(id_resultado) || Number.isNaN(id_dato_macro)) return null;
  return { id_resultado, id_dato_macro };
}

async function main() {
  const defaultInProject = resolve(__dirname, 'data', 'resultados_fuente_macro.csv');
  const defaultDownloads = resolve(process.env.HOME || process.env.USERPROFILE, 'Downloads', 'resultados_fuente_macro.csv');
  const csvPath = process.argv[2] ?? (existsSync(defaultInProject) ? defaultInProject : defaultDownloads);
  console.log('Leyendo CSV:', csvPath);

  if (!existsSync(csvPath)) {
    console.error('No se encuentra el CSV. Usa: node scripts/upload-resultados-fuente-macro.mjs <ruta/al/archivo.csv>');
    process.exit(1);
  }
  ...
  for (let i = 0; i < rows.length; i += BATCH_SIZE) {
    ...
    const { error } = await supabase
      .from('resultados_fuente_macro')
      .upsert(batch, { onConflict: 'id_resultado,id_dato_macro' });
    if (error) {

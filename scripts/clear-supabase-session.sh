...
echo "=== 1. Quitar token de esta terminal (si existía) ==="
unset SUPABASE_ACCESS_TOKEN 2>/dev/null || true

echo "=== 2. Cerrar sesión Supabase CLI ==="
printf 'y\n' | supabase logout 2>/dev/null || supabase logout || true

echo "=== 3. Borrar carpetas locales de credenciales CLI (si existen) ==="
...
echo "   supabase login"
echo "   (NO uses --no-browser ni --token; debe abrir el navegador)"
echo ""
